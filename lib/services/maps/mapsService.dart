...
class mapServices {
  ...
  static Future<bool> isLocationServiceAvailable() async {
    ...
    if (permission == LocationPermission.denied) {
      debugPrint('🗺️ [MAPSERVICE] Permission denied, requesting permission...');
    ...
    if (permission == LocationPermission.deniedForever) {
      debugPrint('🗺️ [MAPSERVICE] ❌ Location permissions are permanently denied');
      return false;
    }
  ...
  static Future<LatLng?> getCurrentUserLocation() async {
    ...
    try {
      ...
      debugPrint('🗺️ [MAPSERVICE] ✅ User location obtained: ${position.latitude}, ${position.longitude}');
      debugPrint('🗺️ [MAPSERVICE] Location details - Accuracy: ${position.accuracy}m, Altitude: ${position.altitude}m');
  ...
  static Stream<LatLng?> getUserLocationStream() {
    ...
    return Geolocator.getPositionStream(
      ...
      debugPrint('🗺️ [MAPSERVICE] 📍 Location stream update: ${position.latitude}, ${position.longitude}');
      debugPrint('🗺️ [MAPSERVICE] Stream details - Accuracy: ${position.accuracy}m, Speed: ${position.speed}m/s');
      return LatLng(position.latitude, position.longitude);
    }).handleError((error) {
  ...
  static Marker? getUserLocationMarker(LatLng? userLocation) {
    if (userLocation == null) {
      debugPrint('🗺️ [MAPSERVICE] ❌ Cannot create user location marker - location is null');
      return null;
    }

    debugPrint('🗺️ [MAPSERVICE] ✅ Creating user location marker for: ${userLocation.latitude}, ${userLocation.longitude}');
  ...
  static CircleMarker? getUserLocationAccuracyCircle(LatLng? userLocation, {double? accuracy}) {
  ...
  static Future<void> centerMapOnUser(MapController mapController, {double zoom = 16.0}) async {
    final userLocation = await getCurrentUserLocation();
    if (userLocation != null) {
      mapController.move(userLocation, zoom);
      debugPrint('🗺️ [MAP_CENTER] Centered map on user location: ${userLocation.latitude}, ${userLocation.longitude}');
    } else {
      debugPrint('🗺️ [MAP_CENTER] Cannot center on user location: location not available');
    }
  }

  /// Centers the map on device location with smooth animation
  static void centerMapOnDevice(MapController mapController, LatLng deviceLocation, {double zoom = 16.0}) {
    mapController.move(deviceLocation, zoom);
    debugPrint('🗺️ [MAP_CENTER] Centered map on device location: ${deviceLocation.latitude}, ${deviceLocation.longitude}');
  }
}
