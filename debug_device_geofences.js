...
async function debugDeviceGeofences() {
  try {
    ...
    const deviceSnapshot = await db.collection('devices').doc('B0A7322B2EC4').get();
    if (deviceSnapshot.exists) {
      ...
      const geofencesForDeviceId = await db.collection('geofences')
        .where('deviceId', '==', 'B0A7322B2EC4')
        .get();
      console.log(`   🗺️ Geofences with deviceId='B0A7322B2EC4': ${geofencesForDeviceId.size}`);
    ...
    const devicesByName = await db.collection('devices')
      .where('name', '==', 'TESTING2')
      .get();
    ...
    if (devicesByName.size > 0) {
      ...
      const geofencesForDeviceName = await db.collection('geofences')
        .where('deviceId', '==', 'TESTING2')
        .get();
      console.log(`   🗺️ Geofences with deviceId='TESTING2': ${geofencesForDeviceName.size}`);
    ...
    const userGeofences = await db.collection('geofences')
    ...
    if (userGeofences.size > 0) {
      userGeofences.forEach(doc => {
        const data = doc.data();
        console.log(`      - ${doc.id}: ${data.name} (deviceId: ${data.deviceId})`);
      });
    }
