...
async function checkCollections() {
  ...
  try {
    ...
    console.log("\n� Devices Collection:");
    const devicesSnapshot = await db.collection("devices").where('ownerId', '==', userId).get();
    console.log(`Found ${devicesSnapshot.size} devices for user`);

    devicesSnapshot.forEach((doc) => {
      const data = doc.data();
      console.log(`- Device ID: ${doc.id}, Name: '${data.name}', Owner: ${data.ownerId}`);
    });
    ...
    if (deviceB0A7.exists) {
      ...
      console.log(`✅ Device found - Name: '${data.name}', Owner: ${data.ownerId}`);
    } else {
    ...
    const geofencesSnapshot = await db.collection("geofences").where('ownerId', '==', userId).get();
    console.log(`Found ${geofencesSnapshot.size} geofences for user`);

    geofencesSnapshot.forEach((doc) => {
      const data = doc.data();
      console.log(`- Geofence ID: ${doc.id}, Name: '${data.name}', DeviceId: '${data.deviceId}', Owner: ${data.ownerId}`);
    });

    // Check geofences specifically for device B0A7322B2EC4
    console.log("\n🔍 Geofences for device B0A7322B2EC4:");
    const geofencesForB0A7 = await db.collection("geofences").where('deviceId', '==', 'B0A7322B2EC4').get();
    console.log(`Found ${geofencesForB0A7.size} geofences with deviceId='B0A7322B2EC4'`);

    // Check geofences for device name TESTING2
    console.log("\n🔍 Geofences for device name TESTING2:");
    const geofencesForTEST2 = await db.collection("geofences").where('deviceId', '==', 'TESTING2').get();
    console.log(`Found ${geofencesForTEST2.size} geofences with deviceId='TESTING2'`);
    ...
    console.log(`Found ${historySnapshot.size} history entries`);    if (historySnapshot.size > 0) {
