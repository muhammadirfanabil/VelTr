...
async function testNotificationDelivery() {
  ...
  try {
    ...
    const usersSnapshot = await db.collection('users_information').limit(1).get();
    ...
    if (!userData.fcmTokens || userData.fcmTokens.length === 0) {
      ...
      console.log('💡 Make sure the user has logged in and FCM tokens are saved');
      return;
    }
    ...
    console.log(`📤 Sending test message to token: ${testToken.substring(0, 20)}...`);
    ...
    try {
      ...
      if (fcmError.code === 'messaging/registration-token-not-registered') {
        console.log('💡 Token is invalid - user needs to re-login to get new token');
    ...
    console.log(`✅ Test notification stored in database: ${notificationRef.id}`);
